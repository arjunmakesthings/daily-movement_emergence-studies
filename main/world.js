class World {
  ...
  */
  ...
  /*
  prevent the possibility of two beings taking up the same space in the world, when beings >2.
  Efficient neighbor-based collision detection using a uniform spatial grid.
*/
  prevent_collisions() {
    const cellSize = 80; // tune this: roughly larger than average being diameter
    const cols = Math.ceil(width / cellSize);
    const rows = Math.ceil(height / cellSize);

    // grid: key -> array of being indices
    const grid = new Map();

    const cellKey = (cx, cy) => `${cx},${cy}`;

    // place each being into a grid cell
    for (let i = 0; i < this.beings.length; i++) {
    ...
    // only check each being against nearby cells
    ...
    for (let i = 0; i < this.beings.length; i++) {
      ...
      for (const [dx, dy] of neighborOffsets) {
        const key = cellKey(acx + dx, acy + dy);
        const bucket = grid.get(key);
        if (!bucket) continue;

        for (const j of bucket) {
          if (j <= i) continue; // avoid double-checking and self-checks
