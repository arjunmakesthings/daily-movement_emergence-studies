class Being {
  constructor(x, y) {
    ...
    this.switch = false;
    this.trail = []; // Store position history
    this.trailLength = 50; // How many points to keep
    this.noiseOffset = random(10000); // Unique noise offset per being
  }
  get_schedule() {}
  exist() {
    this.body();
  ...
  body() {
    // Draw trail
    let colorValue = 255 * sin(frameCount * 0.02 + this.noiseOffset);
    stroke(colorValue);
    strokeWeight((noise(this.pos.x) + noise(this.pos.y))*this.mass);
    noFill();
    beginShape();
  ...
  move() {
    ...
    if (distance < 10) {
      ...
      const possible = [
        createVector(0, 0), // top left
        createVector(width, 0), // top right
        createVector(width, height), // bottom right
        createVector(0, height), // bottom left
        createVector(width / 2, height / 2),
      ];
      this.target = possible[Math.floor(Math.random() * possible.length)];
    }

    desired.normalize();
    desired.mult(this.energy / this.mass); // Scale by energy

    // Separation force from nearby beings
    let separation = createVector(0, 0);
    let separationRadius = 1000; // Increased from 50
    for (let other of world.beings) {
      if (other !== this) {
        let d = p5.Vector.dist(this.pos, other.pos);
        if (d < separationRadius) {
          let away = p5.Vector.sub(this.pos, other.pos);
          away.normalize();
          away.mult(1 / d); // Stronger when closer
          separation.add(away);
        }
      }
    }
    separation.mult(this.energy / 10); // Increased from this.energy / 4
